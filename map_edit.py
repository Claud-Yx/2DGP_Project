...
if __name__ == "__main__":
    ...
    pico2d.open_canvas(gs_framework.canvas_width, gs_framework.canvas_height)
...
def create_map():
    ...
    # stage map
    server.stage = ob_map.Map(1, 1,
                              1200,
                              4400
                              )

    # background =======================================================================================================
    server.background = ob_background.Background(1.2, 1.2, 1.5, 1.5)
...
def write_stage_file():
    with open('map_edit.py', 'r') as f:
        with open("stage\\backup\\s%.2d%.2d.bak" % (server.stage.stage, server.stage.map), 'w') as w:
            w.write(f.read())
...
if __name__ == "__main__":
    ...
    with open('stage\\s%.2d%.2d.stg' % (server.stage.stage, server.stage.map), 'wb') as f:
        object_manager.objects.append(server.stage)
        pickle.dump(object_manager.objects, f)
